...
Simply put, this tool tears a `.vue` file apart, and provides all parts under a single object, so that other JavaScript codes can make use of these parts separately.
...
-   Or say we are writting a Vue component as a lib, but we choose to write this component in TypeScript instead of JavaScript. When a project of pure JavaScript needs to make use of the lib written by us, that project might have difficulties to import our `.vue` file of TypeScript directly. So, as the author of the lib, we should provide a JavaScript version of our `.vue` file besides the oringial TypeScript version. That is, we shall provide both versions and let other developers to choosen either of them. This is a nice example that my tool (well, not exactly mine) plays a role. You write your `.vue` lib component in TypeScript, then you design a simple tool chain to convert the TypeScript version into a JavaScript version. Then obviously my tool can help you build your tool chain.


### Modifications over Original Codes

To satisfy the eslint, I did modify the orginal codes a bit.

1.  Such as, change all this kind of codes:
...
1.  Additionally, I've also changed some function names.
